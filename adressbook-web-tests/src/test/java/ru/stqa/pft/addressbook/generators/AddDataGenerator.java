...
public class AddDataGenerator {
  ...
  private void saveAsCsv(List<AddData> contacts, File file) throws IOException {
    ...
    try (Writer writer = new FileWriter(file)) {
      for (AddData contact : contacts) {
        writer.write(String.format("%s;%s;%s;%s;%s\n", contact.getFerstname(), contact.getLastName(), contact.getEmail1(),contact.getHomePhone()));
      }
    }
  }
  ...
  private static List<AddData> generateContacts(int count) {
    ...
    for (int i = 0; i < count; i++){
      contacts.add(new AddData().withFerstname(String.format("FerstName %s", i)).withLastName(String.format("LastName %s", i)).withEmail1(String.format("Email1 %s", i)).withHomePhone("34534534534"));
    }
    return  contacts;
  }
}
